...
const openNotification = (product) => {
    notification.warning({
        message: 'Thông Báo :',
        description:
            'Đã xóa sản phẩm ' + product + ' khỏi giỏ hàng  !',
    });
}
                        ...
                        cartList.map((cartItem, index) => {
                            ...
                            return (
                                <div className="cart-item">
                                    ...
                                    <div className="price-col"><span>${item.price}</span></div>
                                    <div className="quantity-col">
                                        <button onClick={() => updateCart(item, quantity - 1)}>-</button>
                                        <span>{quantity}</span>
                                        <button onClick={() => updateCart(item, quantity + 1)}>+</button>
                                    </div>
                                    <div className="total-col"><span>${item.price * quantity}</span></div>
                                    <div className="remove-col">
                                        <button className="btn-remove" onClick={() => deleteCart(item)}>
